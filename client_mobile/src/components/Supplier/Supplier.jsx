...
export default function Supplier({ item, sport }) {
  const { screenWidth, favorites, user ,supplierAddFav, messageBack } = useSelector((state) => state);
  ...
  let arrayAux = [];
  item.fields?.forEach((element) => {if (!arrayAux.includes(element.sport)) arrayAux.push(element.sport)});
    ...
    >
      ...
      >
        ...
        <View style={styles.info}>
          ...
          {
            arrayAux.map((el) => <Text style={styles.sportText}>{el}</Text> )
          }
          </View>
