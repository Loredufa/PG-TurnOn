...
export default function CourtCard({ courtInfo }) {
  const { setCurrentCourt, setSection, supplier, setSupplierCourts } =
  ...
  const handleDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/supplier/court/${courtInfo.id}`)
          .then(() => {
            axios
              .get(`/supplier/court/${supplier.id}`)
  ...
  return (
    <Wrapper>
      ...
      <BottomDiv>
        <Buttons1Container>
          <Button onClick={handleSection} value="edit">
            Editar
          </Button>
          <ButtonRed onClick={handleDelete}>Eliminar</ButtonRed>
        </Buttons1Container>
        <Buttons2Container>
          <Button value="calendar" onClick={handleSection}>
            Turnos
          </Button>
          <Button value="availability" onClick={handleSection}>
            Horarios
          </Button>
        </Buttons2Container>
      </BottomDiv>
